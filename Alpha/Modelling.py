...
class Modelling:
    def __init__(self, task_id, index, peptide, cutoff):
        self.index = index
            ...
            try:
                ...
                if os.path.isfile(f"{pep_path}/{code_pdb[0:4]}.pdb"):
                    try:
                        ...
                        for ext in exts:
                            os.rename(
                                f"pep{self.index}.{ext}",
                                f"{pep_path}/pep{self.index}.{ext}",
                            )
    ...
    def __writelog(self, msg, show_code=True, cod=""):
        ...
        with open(f"{path}/out.log", "a") as outfile:
            outfile.write(
                f"pep{self.index}, {self.peptide}, {cod} pident, {msg}, "
